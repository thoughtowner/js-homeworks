...
app.post('/products', async (req, res) => {
    ...
    try {
        if (title && price) {
            ...
            res.status(400).json({ 'error': 'The request body was passed incorrectly: title, price' });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ 'error': error.message });
...
app.put('/consumers/addMoney/:consumerID', async (req, res) => {
    ...
    try {
        if (consumerID, money) {
            ...
            res.status(400).json({ 'error': 'The request params and body was passed incorrectly: money' });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ 'error': error.message });
